...
func handleRequest(w dns.ResponseWriter, query *dns.Msg) {
	...
	// get connection
	// start = time.Now()
	connection, status := network.GetConnectionByDNSRequest(&rAddr.IP, uint16(rAddr.Port), fqdn)
	// log.Tracef("nameserver: took %s to get connection (and maybe process)", time.Since(start))
	switch status {
	case process.NoSocket:
		log.Warningf("nameserver: someone is requesting %s, but could not find socket, returning nxdomain", fqdn)
		nxDomain(w, query)
		return
	case process.NoProcess:
		log.Warningf("nameserver: someone is requesting %s, but could not find process, returning nxdomain", fqdn)
		nxDomain(w, query)
		return
	case process.NoProcessInfo:
		log.Warningf("nameserver: someone is requesting %s, but could not get process info, returning nxdomain", fqdn)
		nxDomain(w, query)
		return
	}
	...
	if connection.Verdict == network.UNDECIDED {
		// start = time.Now()
		sheriff.DecideOnConnectionBeforeIntel(connection, fqdn)
	...
	if connection.Verdict == network.UNDECIDED {
		sheriff.DecideOnConnectionAfterIntel(connection, fqdn, rrCache)
	}
	if connection.Verdict == network.BLOCK || connection.Verdict == network.DROP {
		nxDomain(w, query)
		return
